  ...
  const ProductGrid = () => (
    <div className="space-y-4">
      <div className="relative">
        ...
        <input className="input-pastel pl-11" placeholder="Buscar producto..." value={search} onChange={(e) => setSearch(e.target.value)} />
      </div>
            ...
            <div key={item.product.id} className="p-3 rounded-2xl bg-pastel-lavender/10 space-y-2">
              <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-1">
                  <button className="w-8 h-8 rounded-xl bg-white border flex items-center justify-center" onClick={() => updateQuantity(item.product.id, -1)}>
                  ...
                  <button className="w-8 h-8 rounded-xl text-rose-500" onClick={() => removeFromCart(item.product.id)}>
      ...
      <div className="border-t border-pastel-lavender/30 mt-4 pt-4 space-y-3">
        <div className="space-y-1.5 text-sm p-3 rounded-2xl bg-pastel-cream">
          ...
          <div className="flex items-center gap-2 pt-1">
            ...
            <input
              ...
              onChange={(e) => setGlobalDiscount(Math.min(Number(e.target.value) || 0, afterItemDiscounts))}
              className="input-pastel py-1.5 text-xs flex-1"
            />
          </div>
          {globalDiscount > 0 && (
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <button type="button" onClick={() => setPaymentMethod('nequi')}
              className={`flex items-center justify-center gap-2 py-3.5 rounded-2xl font-bold text-sm min-h-12 transition-all ${
        ...
        <Button variant="success" size="xl" icon={CreditCard} className="w-full" onClick={completeSale} disabled={processing || !cart.length}>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="sm:hidden">
        {mobileTab === 'products' ? <ProductGrid /> : <Card className="p-4"><CartPanel /></Card>}
      </div>

      <div className="hidden sm:grid sm:grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2"><ProductGrid /></div>
        <Card className="p-5 sticky top-4 h-fit"><CartPanel /></Card>
      </div>
