import { createSale, getSaleById, getSales, updateSalePaymentMethod } from '../services/saleService.js';
import { httpStatusFromError, userFacingError } from '../utils/httpErrors.js';

export async function completeSale(req, res) {
  try {
    const { items, payment_method, global_discount, client_sale_id } = req.body;
...
export async function updateSalePayment(req, res) {
  try {
    const sale = await updateSalePaymentMethod(req.params.id, req.body?.payment_method);
    if (!sale) return res.status(404).json({ error: 'Venta no encontrada' });
    res.json(sale);
  } catch (error) {
    const status = error.message.includes('Caja bloqueada')
